...
typedef struct {
	...
	int *M;	// array to use in mergesort method
...
void runBaseLineExperiments(ParProg par){
	size_t k;
	clock_t timeQ;
	float extraB, extraBmaxB, acStlQ, acStlS, acBI, acBImaxB, acI, acM;
	...
	for (k=0; k<par.REPET; k++){
		...
		copyArray(par.stlS, par.n, par.M);
		...
		timeQ = clock();
		mergeSort(par.M, 0, par.n-1);
		acM += (float)(clock() - timeQ)/CLOCKS_PER_SEC;
	...
	cout << "Time stl::qsort = " << 1000*acStlQ/par.REPET<< " ms" << endl; // = quick sort
	cout << "Time stl::sort = " << 1000*acStlS/par.REPET<< " ms" << endl;
	cout << "Time mergesort = " << 1000*acM/par.REPET<< " ms" << endl;
